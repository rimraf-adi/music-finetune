...
| `run_optuna.py` | Uses Optuna to find optimal hyperparameters (hidden layer sizes, learning rate, dropout, etc.) for the classification head and automatically trains the final model |
...
### 2. Hyperparameter Optimization (Recommended)

To find the optimal hyperparameters for the classification head (such as intermediate sizes, dropout rates, and learning rates) and then automatically train the main model:

```bash
uv run python run_optuna.py
```

This script will run 20 optimization trials, save the best parameters to `best_optuna_params.json`, and then invoke `finetune_reward_model.py` to train the final model using these optimal parameters. The final model is saved to `final_reward_model_output`.

### 3. Fine-tune Reward Model (Manual Configuration)

If you prefer to manually configure the hyperparameters without running the optimization step, you can run the finetuning script directly:
...
You can also pass a JSON config (like the one generated by Optuna) to this script:

```bash
uv run python finetune_reward_model.py --config_json best_optuna_params.json
```
...
├── run_optuna.py                    # Stage 2: Optuna hyperparameter search for classification head
├── finetune_reward_model.py         # Stage 3: LoRA fine-tuning with Bradley–Terry loss
├── test_reward_model.py             # Stage 4: Inference & evaluation
