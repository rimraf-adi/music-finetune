...
\subsection{Mitigating Label Noise via Same-Source Pairing and Augmentation}

An initial attempt to pair every original file with every corrupted file (an $\mathcal{O}(n^2)$ Cartesian product) introduced severe label noise: comparing a pristine Chopin piece to a structurally broken Beethoven piece is ill-posed because their underlying musical content is completely mismatched. To resolve this, we enforce strict \textbf{same-source pairing}: an original segment is only ever compared against a corrupted version of itself. 

To prevent data starvation from this strict $1:1$ constraint, we employ corruption augmentation: each original segment $\mathbf{x}_i^{\text{orig}}$ is subjected to $k$ independent stochastic corruptions (by default, $k=5$), yielding a dataset of $\mathcal{O}(k \cdot n)$ valid pairs:

\begin{equation}
    \mathcal{D} = \bigl\{\,(\mathbf{x}_i^{\text{orig}},\;\mathbf{x}_{i,c}^{\text{corrupt}},\;y=1) \;\big|\; i \in \{1,\ldots,n\}, c \in \{1,\ldots,k\}\,\bigr\}
\end{equation}

This guarantees that the preference label is directly tracking the presence of structural coherence rather than spurious stylistic differences between different composers or pieces.
...
We target the self-attention \textbf{Query} and \textbf{Value} matrices of the \textbf{last 4 layers} of the 12-layer encoder (layers 8 through 11). For $r_{rank}=8$ and $\alpha=16$, this yields 8 adapter pairs and $98{,}304$ LoRA parameters in total. Including the reward head, the trainable parameter count in our implementation is $313{,}217$ out of $110{,}842{,}626$ total ($\approx 0.28\%$), balancing training capacity with efficiency.
...
To prevent the model from collapsing to trivial, infinitesimally small reward differences, we modify the standard Bradley-Terry loss by introducing a scalar \textbf{target margin} $m > 0$ (in our configuration, $m=1.0$). The network minimizes the margin-adjusted negative log-likelihood:

\begin{equation}
    \mathcal{L}_{\text{RM}} = - \mathbb{E}_{(\mathbf{x}^w, \mathbf{x}^l) \sim \mathcal{D}} \left[ \log \sigma\bigl(r(\mathbf{x}^w) - r(\mathbf{x}^l) - m\bigr) \right]
\end{equation}

This loss forces the RM to not only rank the original sequence higher than its corrupted counterpart, but to confidently push the scalar reward gap $r(\mathbf{x}^w) - r(\mathbf{x}^l)$ to be at least $m$.
...
\begin{itemize}
    ...
    \item \textbf{LoRA Sub-matrices:} Rank $r = 8$, applied to the Query and Value projections of the final 4 layers. Learnable parameters $\Delta W = B A$, where $B \in \mathbb{R}^{768 \times 8}$ and $A \in \mathbb{R}^{8 \times 768}$.
